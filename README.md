...
Bộ test: 80 test. Trong bản clone sạch: **70 pass, 10 skip** — 10 test cần dữ liệu không kèm repo (4 hồ sơ 10-K của Phụ lục A.2, bản PRC đầy đủ, index EDGAR, từ điển Loughran–McDonald, cache EDGAR, test mạng). Sau khi notebook tải hai thứ đầu: **75 pass, 5 skip**. Trên máy có đủ dữ liệu: 79 pass và 1 test mạng.
...
cyberrisk/
  ...
  tests/                               80 test (bản clone sạch: 70 pass, 10 skip vì thiếu dữ liệu lớn)
...
python -m pytest cyberrisk/tests -q                    # bản clone sạch: 70 pass, 10 skip (thiếu dữ liệu lớn)
